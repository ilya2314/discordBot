...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const Embed = new EmbedBuilder().setTitle("🔊 Voice System").setColor(client.color).setTimestamp(Date.now());
        const ownedChannel = client.voiceGenerator.get(member.id);

        if (!voiceChannel)
            return interaction.reply({ embeds: [Embed.setDescription("🔇 You are currently not in a voice channel.").setColor(client.color)], ephemeral: true });

        if (!ownedChannel || voiceChannel.id !== ownedChannel)
            return interaction.reply({ embeds: [Embed.setDescription("❌ You do not own this channel!").setColor(client.color)], ephemeral: true })

        switch (subCommand) {
            case "name": {
                const newName = options.getString("text");
                if (newName.length > 22 || newName.length < 1)
                    return interaction.reply({ embeds: [Embed.setDescription("❌ Name cannot exceed the 22 character limit").setColor(client.color)], ephemeral: true })
            ...
            case "invite": {
                ...
                const sendEmbed = new EmbedBuilder().setTitle("🔊 Voice System").setColor(client.color).setDescription(`👋 <@${member.id}> has invited you to <#${voiceChannel.id}>`)
